...
class VillageService {
  ...
  Future<List<Dease>> fetchDeases({required BuildContext context}) async {
    List<Dease> deasesList = [];
    try {
      http.Response res =
          await http.get(Uri.parse("$uri/account/deases/"), headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      });
      httpErrorHandle(
          response: res,
